...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const checkAgentConnection = async (userEmail: string) => {
    try {
      const connectionsRef = collection(db, "viewerAgentConnections")
      const q = query(connectionsRef, where("viewerEmail", "==", userEmail), where("status", "==", "Active"))
      const snapshot = await getDocs(q)

      if (!snapshot.empty) {
        const connectionData = snapshot.docs[0].data()
        setConnectedAgent(connectionData.agentName)
        console.log(`[v0] Viewer ${userEmail} connected to agent: ${connectionData.agentName}`)
        return connectionData.agentName
      } else {
        setConnectedAgent(null)
        console.log(`[v0] No agent connection found for viewer: ${userEmail}`)
        return null
      }
    } catch (error) {
  ...
  useEffect(() => {
        ...
        if (firebaseUser) {
          try {
            ...
            if (userDoc.exists()) {
              ...
              if (userData.role === "Viewer" && firebaseUser.email) {
                await checkAgentConnection(firebaseUser.email)
              } else {
                setConnectedAgent(null)
              }
            } else {
    ...
    return () => unsubscribe()
  }, []) // Removed initialLoadComplete dependency
  ...
  return (
    <AuthContext.Provider
      value={{
        ...
        connectedAgent,
        isAgentViewer: !!connectedAgent && user?.role === "Viewer",
        changePassword,
        resetPassword,
        loginError,
        loading,
